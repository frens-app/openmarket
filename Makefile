...
ios-settings: ios-generate
	@for cfg in Debug Release; do \
		...
		cd apps/ios && xcodebuild -project OpenMarket.xcodeproj -scheme OpenMarket \
			...
			grep -E '^\s+(PRODUCT_BUNDLE_IDENTIFIER|DISPLAY_NAME|API_SCHEME|API_HOSTNAME|APNS_ENVIRONMENT|MARKETING_VERSION|CURRENT_PROJECT_VERSION|POSTHOG_API_KEY|POSTHOG_HOSTNAME) ' | \
			sed 's/^ */  /'; cd ../..; \
	done
